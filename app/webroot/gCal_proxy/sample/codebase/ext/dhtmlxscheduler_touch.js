...
scheduler.showCover=function(box){
	...
	var tid = node.addEventListener('webkitTransitionEnd', function(event) { 
		node.style.top = "41px";
		node.style.webkitTransform = "";
		node.style.webkitTransition = "";
		node.removeEventListener(tid);
	}, false );
...
scheduler.hideCover=function(box){
	if (box){
		var tid = box.addEventListener('webkitTransitionEnd', function(event) { 
			box.style.top = ((box.offsetHeight+41)*-1)+"px";
			box.style.webkitTransform = "";
			box.style.webkitTransition = "";
			box.removeEventListener(tid);	
		}, false );
