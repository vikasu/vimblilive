...
<section id=header-wrap style="background:url('<?php echo SITE_URL; ?>img/inner_header_bg.png') repeat-x scroll left bottom transparent; height:44px;">
  <!-- Top menu :: Begin -->
    <?php
      if(@$_SESSION['Auth']['User']['user_type'] == 1){
        echo $this->element("dashboard/individual_header");  
      }else if(@$_SESSION['Auth']['User']['user_type'] == 2){
        echo $this->element("dashboard/group_header");  
      } else{
        echo $this->element("home_layout/menu");
      }
    ?>
  <!-- Top menu :: End -->
</section>
...
<?php
      if(($_SESSION['Auth']['User']['user_type'] != "") || ($this->params['action']=='sign_up') || ($this->params['action']=='login')){
        echo $this->element("dashboard/group_footer"); 
      } else{
        echo $this->element("home_layout/footer");
      }
    ?>
