...
var $separator    = '<span>&raquo;</span>'            ;    // separator between links.
var $protocol    = 'http'            ;
...
function getHtml($title = null, $what_to_do=null, $level = null)
{
    ...
    for ($i = 0  ; $i <= $last_index  ; $i++) {
        ...
        if ($i < $last_index) { // no need to build link for last item
            ...
            $link = sprintf("<span class='%s'>%s</span>", $this->span_class, '<strong>'.$title.'</strong>');//last text, ie current page without link
        }
        $string    .=  $link                            ;
    }
    return $string;
