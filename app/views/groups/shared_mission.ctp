             ...
             <section class=whiteboxmid>
                 <section class=whiteboxmid-right>
                      ...
                      <section class=aboutpane-inner>
                           ...
                           <section class=dshbrd-right>
                               ...
                               <section class="current-mission manggrpdsbrd">
                                    <!--Heading-->
                                    <h3 class=wrdspcn>Shared <span>Mission</span>
					<div class="addimprt-btns">
					    <a class="blubtn-mid" href="<?php echo SITE_URL ?>missions/current_mission_setup">
					    <input type="button" value="Add Mission" />
					    </a>
					</div>
					...
					<div class="cnnctns-actns"><input class='actionButton' id="delete" type='button' name="data[Connection][action]" value="Delete"></div>
                                        ...
                                        -->
					<?php /* ?>
                                   <form action="<?php echo SITE_URL ?>timelines/index/<?php echo base64_encode($_SESSION['Auth']['User']['id']) ?>" method="post" id="refineForm" name="refineForm">
                                        <div style="float:right;" class="slctsrch">
                                             <div class="slctbx"style="width:130px; margin: 0 0 3px;">
                                                <select id="entity_options" name="data[Timeline][entity_type]">
						    <option value="all" <?php if($this->data['Timeline']['entity_type'] == 'all'){ ?>selected="selected"<?php } ?>>All Activities</option>
						    <option value="calendar" <?php if($this->data['Timeline']['entity_type'] == 'calendar'){ ?>selected="selected"<?php } ?>>Calendar Events</option>
						    <option value="email" <?php if($this->data['Timeline']['entity_type'] == 'email'){ ?>selected="selected"<?php } ?>>Emails</option>
						    <option value="reflection" <?php if($this->data['Timeline']['entity_type'] == 'reflection'){ ?>selected="selected"<?php } ?>>Reflections</option>
						    <option value="activity"  <?php if($this->data['Timeline']['entity_type'] == 'activity'){ ?>selected="selected"<?php } ?>>Added Activities</option>
						    <option value="other" <?php if($this->data['Timeline']['entity_type'] == 'other'){ ?>selected="selected"<?php } ?>>Other</option>
						</select>
						 <!--
						...
						<li>
						    <section class="mng-actvty">
						       <?php if($row['Mission']['title'] != ''){
							    echo substr($row['Mission']['title'],0,40);
							    if($row['Mission']['edited_by'] != 0){ echo ' <i>(edited)</i>'; }
						     } else {
							  echo 'N/A';
						     }
						     ?>
						    ...
						    <section class="crntprgrss-actn">
							    <a href="<?php echo SITE_URL ?>missions/copy_shared_mission/<?php echo $row['Mission']['id'] ?>"><img src="<?php echo SITE_URL ?>img/copy_icon.png" class="image_icon" title="Copy" alt="Copy"></a>
							    <a href="<?php echo SITE_URL ?>groups/view_mission/<?php echo base64_encode($row['Mission']['id']) ?>"><img src="<?php echo SITE_URL ?>img/mission_icon.png" class="image_icon" title="Mission" alt="Mission"></a>
