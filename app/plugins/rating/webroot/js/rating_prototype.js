/**
 * Prototype javascript for the rating plugin.
 *
 ...
 * @param webroot Webroot of the application
 */
function ratingInit(element, data, config, options, enabled, webroot) {
  ...
  ratingSettings[element]['webroot'] = webroot;
...
function ratingSet(element, value) {
  ...
  for (i = 1; i <= ratingSettings[element]['data']['%MAX%']; i++) {
    ...
    $(element + '_' + i).src = ratingSettings[element]['webroot'] + starImg;
...
function ratingSave(element, value) {
  ...
  if (ratingSettings[element]['enabled']) {
    new Ajax.Updater(element, ratingSettings[element]['webroot'] + 'rating/ratings/save/' + data[0] + '/' + data[3] + '/' + value + '?' + Math.floor(Math.random() * 999999), {               
      asynchronous: true,
      evalScripts: true,
      method: 'get',
