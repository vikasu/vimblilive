/**
 * jQuery javascript for the rating plugin.
 *
...
function ratingSet(element, value) {
  ...
  for (i = 1; i <= ratingSettings[element]['data']['%MAX%']; i++) {
    ...
    $('#' + element + '_' + i).attr({'src': ratingSettings[element]['webroot'] + starImg});
...
function ratingSave(element, value) {
  ...
  if (ratingSettings[element]['enabled']) {
    $.ajax({
      url: ratingSettings[element]['webroot'] + 'rating/ratings/save/' + data[0] + '/' + data[3] + '/' + value + '?' + Math.floor(Math.random() * 999999),
      async: true,
