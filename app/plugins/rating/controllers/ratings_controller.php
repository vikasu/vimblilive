<?php
/**
 * Controller for the rating plugin.
 *
...
class RatingsController extends RatingAppController {
  ...
  function view($model, $id, $options) {
    ...
    $settings = Configure::read($model.$name.$id.'RatingConfig');
    
    // save overload settings in session (for ajax requests)
    if (!empty($settings)) {
      $this->Session->write($model.$name.$id.'RatingConfig', $settings);
    } else {
      $settings = $this->Session->read($model.$name.$id.'RatingConfig');
    }
    
    // load config
    $this->__loadConfig($config, $settings);
  ...
  /**
   * Loads a config file and overloads settings.
   * 
   * @param $file Name of the config file
   * @param $settings Settings to overload
   */
  function __loadConfig($file, $settings = array()) {
    ...
    if (!empty($settings)) {
      foreach ($settings as $key => $setting) {
        if (is_array(Configure::read($key))) {
          Configure::write($key, am(Configure::read($key), $setting));
        } else {
